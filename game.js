window.addEventListener('DOMContentLoaded', () => {
    ...
    const Models = {
        buildAlexa() {
            ...
            // ===== ТОРС: приталенная куртка-жилет (профиль вращения), перекрашивается бронёй =====
            ...
            torso.scale.z = 0.82; torso.castShadow = true; root.add(torso);
            ...
            torso.add(M(new THREE.BoxGeometry(0.16, 0.12, 0.06), Mats.metalDark, 0, 1.95, 0.42));            // пряжка
            [-1, 1].forEach(s => { const st = M(new THREE.BoxGeometry(0.09, 1.05, 0.05), Mats.strap, s * 0.18, 2.55, 0.4); st.rotation.z = s * 0.5; torso.add(st); }); // перекрёстные ремни
            torso.add(M(new THREE.BoxGeometry(0.26, 0.2, 0.06), Mats.pad, -0.22, 2.42, 0.36));               // нагрудный модуль
            ...
            backpack.add(M(new THREE.CylinderGeometry(0.015, 0.015, 0.6, 5), Mats.metalDark, 0.28, 0.5, -0.1));   // антенна
            backpack.add(M(new THREE.SphereGeometry(0.04, 8, 8), Mats.neon, 0.28, 0.82, -0.1));                   // огонёк антенны
            const flashLight = new THREE.PointLight(0x9fe6ff, 0, 26, 1.6); flashLight.position.set(0, 0.2, -0.4); backpack.add(flashLight);
            [-1, 1].forEach(s => torso.add(M(new THREE.BoxGeometry(0.08, 1.0, 0.06), Mats.strap, s * 0.2, 2.55, 0.46))); // лямки спереди

            // ===== ШЕЯ + ГОЛОВА + РЫЖИЙ ХВОСТ + ОЧКИ НА ЛБУ =====
            ...
            head.add(M(new THREE.ConeGeometry(0.05, 0.12, 6), Mats.skin, 0, -0.02, 0.3));                          // нос
            head.add(M(new THREE.SphereGeometry(0.33, 18, 14, 0, Math.PI * 2, 0, Math.PI * 0.62), Mats.hair, 0, 0.06, -0.02)); // шапка волос
            head.add(M(new THREE.SphereGeometry(0.31, 16, 10, 0, Math.PI * 2, 0, Math.PI * 0.32), Mats.hair, 0, 0.14, 0.08));  // чёлка
            ...
            const p2 = M(new THREE.CylinderGeometry(0.09, 0.04, 0.5, 8), Mats.hair, 0, -0.62, 0.04); p2.rotation.x = 0.25; pony.add(p2);
            ...
            // ===== РУКИ (sr/sl анимируются) =====
            const buildArm = (side) => {
                const sh = new THREE.Group(); sh.position.set(side * 0.46, 2.92, 0); torso.add(sh);
                sh.add(M(new THREE.SphereGeometry(0.18, 12, 10), Mats.pad, 0, 0.04, 0));                  // наплечник
                sh.add(M(new THREE.CylinderGeometry(0.13, 0.115, 0.7, 10), Mats.sleeve, 0, -0.42, 0));    // плечо
                sh.add(M(new THREE.CylinderGeometry(0.105, 0.095, 0.62, 10), Mats.sleeve, 0, -1.05, 0.02)); // предплечье
                sh.add(M(new THREE.SphereGeometry(0.13, 12, 10), Mats.glove, 0, -1.42, 0.04));            // кисть в перчатке
                return sh;
            };
            const shR = buildArm(1), shL = buildArm(-1);
            const anc = new THREE.Group(); anc.position.set(0.04, -1.45, 0.16); shR.add(anc);             // якорь оружия = правая кисть

            // ===== ТАЗ + НОГИ (lr/ll анимируются) =====
            torso.add(M(new THREE.CylinderGeometry(0.42, 0.36, 0.4, 14), Mats.pants, 0, 1.62, 0));
            torso.add(M(new THREE.BoxGeometry(0.18, 0.24, 0.14), Mats.strap, 0.34, 1.55, 0.18));          // бедренный подсумок
            const buildLeg = (side) => {
                const hip = new THREE.Group(); hip.position.set(side * 0.2, 1.6, 0); torso.add(hip);
                hip.add(M(new THREE.CylinderGeometry(0.19, 0.15, 0.82, 10), Mats.pants, 0, -0.42, 0));    // бедро
                hip.add(M(new THREE.SphereGeometry(0.13, 10, 8), Mats.pad, 0, -0.82, 0.12));              // наколенник
                hip.add(M(new THREE.CylinderGeometry(0.13, 0.11, 0.66, 10), Mats.pants, 0, -1.15, 0.01)); // голень
                hip.add(M(new THREE.BoxGeometry(0.22, 0.24, 0.46), Mats.boots, 0, -1.5, 0.08));           // ботинок
                hip.add(M(new THREE.BoxGeometry(0.24, 0.08, 0.5), Mats.boots, 0, -1.6, 0.09));            // подошва
                return hip;
            };
            const hipR = buildLeg(1), hipL = buildLeg(-1);

            scene.add(root);
            return { root, torso, sr: shR, sl: shL, lr: hipR, ll: hipL, anc, neon, flashLight, head, pony };
        },
        buildWeapon(wId) {
